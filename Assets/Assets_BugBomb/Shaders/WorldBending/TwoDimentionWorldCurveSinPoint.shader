...
Shader "WiedzowyWtorek/TwoDimentionWorldCurveSinPoint" {
    Properties {
        // Diffuse texture
        ...
        _CurvatureY ("CurvatureY", Float) = 0.001
			_SinMultX("SinMultX", Float) = 1
			_SinMultY("SinMultY", Float) = 1
    ...
    SubShader {
		...
		uniform float _SinMultX;
		uniform float _SinMultY;
        ...
        void vert( inout appdata_full v)
        {
			...
			//vv = float4((vv.z * vv.z) * -_CurvatureX, sin(_SinMultX * (vv.z) * -_CurvatureY), 0.0f, 0.0f);
			vv = float4(sin(_SinMultY * (vv.z) * -_CurvatureX), sin(_SinMultX * (vv.z) * -_CurvatureY), 0.0f, 0.0f);
